...
export class EosService {
  ...
  getDeferTransaction(id: string): Observable<any> {
    return defer(() => from(this.eos.getTransaction(id, 0)));
  }
  ...
  getAccountTokens(name: string): Observable<Result<any[]>> {
    ...
    const getCurrencyBalance = function(
      ...
      return from(
        this.eos.getCurrencyBalance(token.account, account, token.symbol)
      ).pipe(
        map((balance: string[]) => ({
          ...token,
    ...
    const accountTokens$ = allTokens$.pipe(
      switchMap(tokens => {
        console.log("inside accountTokens : " + tokens);
