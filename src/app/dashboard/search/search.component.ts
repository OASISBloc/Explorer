  ...
  private tryBlockNumber(query: string): Observable<string> {
    ...
    if (!isNaN(blockNumber)) {
      return this.eosService.getDeferBlock(blockNumber).pipe(
        ...
        switchMap(data => {
          if (data) {
            this.router.navigate(["/blocks", blockNumber], {
              replaceUrl: true
            });
            return empty();
          }
        ...
        switchMap(transaction => {
          if (transaction) {
            this.router.navigate(
              ["/transactions", transaction.block_num, transaction.id],
              { replaceUrl: true }
            );
            return empty();
          }
  ...
  private tryBlockId(query: string): Observable<string> {
    if (query.length === 64) {
      return this.eosService.getDeferBlock(query).pipe(
        ...
        switchMap(block => {
          if (block) {
            this.router.navigate(["/blocks", block.block_num], {
              replaceUrl: true
            });
            return empty();
          }
  ...
  private tryAccount(query: string): Observable<string> {
    if (query.length <= 12) {
      return this.eosService.getDeferAccount(query).pipe(
        ...
        switchMap(account => {
          if (account) {
            this.router.navigate(["/accounts", account.account_name], {
              replaceUrl: true
            });
            return empty();
          }
